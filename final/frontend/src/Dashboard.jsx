          ...
          {notifs.length === 0 ? (
            ...
            <div className="divide-y">
              {notifs.slice(0, 20).map((n, idx) => (
                <div key={n.id ?? idx} className={`flex items-start gap-3 px-6 py-4 ${!n.is_read ? "bg-blue-50/40" : ""}`}>
                  ...
                  {!n.is_read && <span className="mt-1 h-2 w-2 rounded-full bg-blue-500 shrink-0" />}
                </div>
              ))}
            </div>
          )}
