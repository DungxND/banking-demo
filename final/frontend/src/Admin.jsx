      ...
      <div className="w-full max-w-lg rounded-2xl border bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        {!detail ? (
          ...
          <div className="px-6 py-5 space-y-5">
            ...
            {/* transfers */}
            <div>
              <div className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">Recent Transfers</div>
              {detail.transfers.length === 0 ? (
                <div className="rounded-xl bg-slate-50 px-4 py-4 text-sm text-center text-slate-400">No transfers</div>
              ) : (
                <div className="max-h-56 overflow-y-auto space-y-2 pr-1">
                  {detail.transfers.map((t) => (
                    <div key={t.id} className={`flex items-center justify-between rounded-xl border px-4 py-2.5 text-sm ${
                      t.direction === "in" ? "border-emerald-200 bg-emerald-50" : "border-red-100 bg-red-50"
                    }`}>
                      <div className="flex items-center gap-2">
                        <span className={`text-base font-bold ${t.direction === "in" ? "text-emerald-600" : "text-red-500"}`}>
                          {t.direction === "in" ? "↓" : "↑"}
                        </span>
                        <div>
                          ...
                          <span className="ml-2 text-xs text-slate-500">
                            {t.direction === "in" ? `from #${t.from_user}` : `to #${t.to_user}`}
                          </span>
                        </div>
                      </div>
...
export default function Admin({ onBack }) {
  ...
  const loadTransfers = useCallback(async (s, p) => {
    try {
      ...
      setTransfers(d.transfers); setTransfersTotal(d.total); setTransfersPages(d.pages);
    } catch { setTransfers([]); }
  }, []);
  ...
  const loadNotifications = useCallback(async (s, p) => {
    try {
      ...
      setNotifications(d.notifications); setNotificationsTotal(d.total); setNotificationsPages(d.pages);
          ...
          <>
            ...
            {stats && (
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
                <StatCard label="Total Users"      value={stats.total_users?.toLocaleString()}             accent="blue" />
                <StatCard label="Total Balance"    value={fmt(stats.total_balance)}                        accent="emerald" />
                <StatCard label="Transfers"        value={stats.total_transfers?.toLocaleString()}         accent="violet" />
