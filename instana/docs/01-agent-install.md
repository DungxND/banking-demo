# Instana Agent Install ΓÇö Linux EC2 (k3s host)
...
> Condensed for: Ubuntu EC2 running k3s (banking-demo phase1)
...
From the Instana UI: **Agents & Collectors ΓåÆ Install agents ΓåÆ Linux ΓÇô Automatic Installation (One-liner)**
...
|------|----------|-------------|
| `-a` | yes | Agent key (from Instana UI ΓåÆ Settings ΓåÆ Agents) |
...
Paste the script into **EC2 ΓåÆ Instance ΓåÆ User data** to auto-install on each launch.
...
After ~30 s the host appears in **Instana UI ΓåÆ Infrastructure**.
...
> The next-gen k8sensor (agent ΓëÑ 1.2.x) still requires `enabled: true` when the agent runs on the host (not inside the cluster). Only an in-cluster Helm/Operator install can use `enabled: false`.
